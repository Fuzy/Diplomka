...
TOTO Deployment diagram

%\section{Architektura (smazat)}
...
\item číslo datového souboru, kde se záznam nachází. Číslo souboru je relativní vzhledem k \emph{tablespace} 
\end{itemize}
Pomocí ROWID lze tedy jednoznačně identifikovat záznam. Bohužel kvůli k tomu, že jeho hodnota je relativní vzhledem k \emph{tablespace}, tak jeho použití jako unikátního identifikátoru může selhat v těchto situacích:
\begin{itemize}[noitemsep,nolistsep]
\item migrace
\item smazání
\end{itemize}

Vhodným řešením by byla změna databázového schématu tabulky pro záznam docházkových událostí a to přidáním primárního klíče.
\subsubsection*{Schéma pro docházku}
+ snazsi zamezit chybnemu vstupu
...
[TODO ještě upřesnit+UML use case?]
