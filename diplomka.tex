...
Získávání přesné polohy zařízení je komplikovaná záležitost. Do kvality získané polohy se podílejí tyto faktory:
\begin{itemize}[]
...
\end{itemize}
[TODO +příklad použití, alg detekce prit/neprit], android dulezite tridy
...
\section{Docházka}
[TODO daytimeline 2x na stejnych datech, casova osa, neukonceno]
...
\centering
  ...
  \caption{Seznam docházkových událostí pro zvolený den řazených dle času}
  \label{fig:daytimelinealt}
\end{minipage}
\end{figure}
Hlavní obrazovka aplikace zobrazuje grafický přehled docházkových událostí pro daný den (obr. \ref{fig:daytimeline}). Zobrazuje se blok jakkékoli přítomnosti a blok vybrané nepřítomnosti (oběd, služebně, lékař). Na obrázku je zobrazena přítomnost v práci od 8-12 hod následována odchodem na oběd ve 12 hod, příchodem z oběda ve 13 hod a následně dosud nukončená přítomnost v práci. Alternativně lze události docházky zobrazit jako seznam jednostlivých událostí (obr. \ref{fig:daytimelinealt}). Každá položka je ohraničena rámečkem jehož barva reprezentuje stav synchronizace.
\noindent
Barvy rámečku:
\begin{itemize}[noitemsep,nolistsep]
...
\end{minipage}
\end{figure}
Tlačítkem se znakem '\emph{+}' (\ref{fig:daytimeline} či \ref{fig:daytimelinealt}) lze zadat lze zadat novou událost. Na obrazovce \ref{fig:edit} lze zvolit parametry události (čas, typ, poznámka) a rovněz lze události smazat pomocí tlačítka '\emph{popelnice}'. Mazání se potvrzuje pomocí dialogu (\ref{fig:delete}). Zadané údaje lze uložit pomocí tlačítka '\emph{disketa}' 

\section{Menu}
Pomocí tlačítka v horním pravém rohu ('\emph{trojice čtverců}' lze zobrazit menu aplikace (obr. \ref{fig:menu}).
...
\item  \textbf{Výkazy}\\zobrazenívýkazů práce (viz. sekce \ref{sec:records})
...
\item  \textbf{Přítomní zaměstnanci}\\zozbrazení seznamu zaměstnanců a jejich stav přítomnosti (viz. sekce \ref{sec:emps})
...
\label{sec:charts}
Modul pro grafy a statistiky je dostupný jak pro docházku tak pro výkazy. Zobrazuje data pro vybraného uživatele za daného období. Lze prohlížet data uložená offline (od předchozího dotazu) nebo aktuální data pomocí tlačíka '\emph{Aktualizovat}'. Mezi jednotlivými obrazovkami lze přepínat pomocí tlačítka '\emph{Přepnout zobrazení}'. Ovládací panel pro volbu zaměstnance a období je popsán v sekci \ref{sec:panel}
\begin{itemize}[noitemsep,nolistsep]
...
Uživatelský účet lze vytvořit v nastavení Android zařízení v sekci '\emph{Nastavení}' a dále tlačítko '\emph{Přidat účet}' nebo potvrzením dialogu s výzvou k přidání účtu v aplikaci. Po výběru účtu  '\emph{Imisoid}' je zobrazena obrazovka (obr. \ref{fig:accact}) pro přidání přihlašovacích údajů. Předpokladem je fungující připojení k serveru pro aplikaci (\ref{fig:network}). Po úspěšném vytvoření účtu se zobrazí dialog \ref{fig:accokdialog})
\begin{figure}[H]
\centering
\begin{minipage}{.45\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{scr/setting.png}
  \captionof{figure}{Nastavení}
  \label{fig:setting}
\end{minipage}\hfill%
\begin{minipage}{.45\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{scr/addacc.png}
...
\begin{minipage}{.45\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{scr/accact.png}
  \captionof{figure}{Zadání přihlašovacíh údajů}
  \label{fig:accact}
\end{minipage}\hfill%
\begin{minipage}{.45\textwidth}
  \centering
  \includegraphics[width=.9\linewidth]{scr/accokdialog.png}
